...
const toTitleCase = (arr) => {
  ...
  return arr.map(value => {
    let makeUpperCase = value.charAt(0).toUpperCase()
    return value.replace(value.charAt(0), makeUpperCase)
  })
};
...
let biggerThanLuke = (arr) => {
  ...
  return arr.map(element =>{
    let toInt = parseInt(element.mass).filter(ele => {
      if (ele.mass > ele.mass){
        console.log(toInt)
        console.log(ele)
      }
    })
